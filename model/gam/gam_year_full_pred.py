...
for k in [15, 20, 25]:
    ...
    for fold in folds:
        ...
        # Scale only continuous columns (0-5), leave Year (6) unscaled
        ...
        # Year column (index 6) remains unscaled
        gam = GammaGAM(
            s(0, n_splines=k) + s(1, n_splines=k) + s(2, n_splines=k) +
            s(3, n_splines=k) + s(4, n_splines=k) + s(5, n_splines=k) + f(6)
...
plot_gam_terms_plotly(
    ...
    plot_title=f"GAM Partial Dependence Analysis - Year (n_splines={best_k})",
...
plot_gam_combined_dashboard_plotly(
    ...
    titles=all_feature_names,
    plot_title=f"GAM Combined Analysis - Year - Train RMSE: {train_rmse:.4f} | Test RMSE: {test_rmse:.4f}",
...
plot_gam_feature_importance_plotly(
    ...
    plot_title=f"GAM Feature Importance Analysis - Year (EDOF)",
