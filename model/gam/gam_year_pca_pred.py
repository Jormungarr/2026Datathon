...
EXPERIMENT_PREFIX = "year_pca_3comp"
N_COMPONENTS = 3
...
all_feature_names = pca_feature_names + time_features  # PC1, PC2, PC3, Year
...
for k in [15, 20, 25]:
    ...
    for fold in folds:
        ...
        X_train_cat = X_train[:, n_numeric:n_numeric+1]  # Year column only
        ...
        # Combine PCA components + categorical Year
        X_train_gam = np.hstack([X_train_pca, X_train_cat])
        X_val_gam = np.hstack([X_val_pca, X_val_cat])

        # GAM: spline terms for PCs + categorical term for Year
        gam = GammaGAM(
...
X_all_cat = X_all[:, n_numeric:n_numeric+1]   # Year column
...
# 1. Partial dependence plots for PC1, PC2, PC3, Year
plot_gam_terms_plotly(
    gam,
    titles=all_feature_names,
    plot_title=f"GAM Partial Dependence - PCA + Year ({N_COMPONENTS} components, n_splines={best_k})",
...
plot_gam_combined_dashboard_plotly(
    ...
    titles=all_feature_names,
    plot_title=f"GAM PCA + Year Combined Analysis - Train RMSE: {train_rmse:.4f} | Test RMSE: {test_rmse:.4f}",
...
plot_gam_feature_importance_plotly(
    ...
    plot_title=f"GAM PCA + Year Feature Importance (EDOF) - {N_COMPONENTS} Components",
...
print("="*80)
print(f"✅ PCA + Year Combined Analysis Complete!")
