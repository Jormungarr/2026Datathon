...
EXPERIMENT_PREFIX = "year_quarter_pca_3comp"
N_COMPONENTS = 3

# Define feature columns
feature_names = ['passengers', 'nsmiles', 'rl_pax_str', 'tot_pax_str', 'large_ms', 'lf_ms']
time_features = ['Year', 'quarter']
pca_feature_names = [f'PC{i+1}' for i in range(N_COMPONENTS)]
all_feature_names = pca_feature_names + time_features  # PC1, PC2, PC3, Year, quarter
...
for k in [15, 20, 25]:
    ...
    for fold in folds:
        ...
        X_train_cat = X_train[:, n_numeric:n_numeric+n_cat]  # Year + quarter columns
        X_val_cat = X_val[:, n_numeric:n_numeric+n_cat]
        ...
        # Combine PCA components + categorical Year + quarter
        # Layout: [PC1, PC2, PC3, Year, quarter] → indices [0, 1, 2, 3, 4]
        X_train_gam = np.hstack([X_train_pca, X_train_cat])
        X_val_gam = np.hstack([X_val_pca, X_val_cat])

        # GAM: spline terms for PCs + categorical terms for Year and quarter
        gam = GammaGAM(
...
X_all_cat = X_all[:, n_numeric:n_numeric+n_cat]    # Year + quarter columns
X_test_cat = X_test[:, n_numeric:n_numeric+n_cat]
...
# 1. Partial dependence plots for PC1, PC2, PC3, Year, quarter
plot_gam_terms_plotly(
    gam,
    titles=all_feature_names,
    plot_title=f"GAM Partial Dependence - PCA + Year + Quarter ({N_COMPONENTS} components, n_splines={best_k})",
...
plot_gam_combined_dashboard_plotly(
    gam,
    X_all_gam, y_all, y_train_pred,
    X_test_gam, y_test, y_test_pred,
    titles=all_feature_names,
    plot_title=f"GAM PCA + Year + Quarter - Train RMSE: {train_rmse:.4f} | Test RMSE: {test_rmse:.4f}",
...
plot_gam_feature_importance_plotly(
    ...
    plot_title=f"GAM PCA + Year + Quarter Feature Importance (EDOF) - {N_COMPONENTS} Components",
...
print("="*80)
print(f"✅ PCA + Year + Quarter Combined Analysis Complete!")
